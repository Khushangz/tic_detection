...
def patch_model_yaml(args) -> None:
    ...
    if args.hidden is not None:
        cfg["bilstm"]["hidden_size"] = int(args.hidden)

    with open(MODEL_YAML, "w") as f:
        yaml.dump(cfg, f, default_flow_style=False, sort_keys=False)

    print(f"[cli] model.yaml patched:")
    print(f"      loss={cfg.get('loss')}  lr={cfg.get('lr')}  "
...
def run_stage(script: str, exp: str) -> None:
...
def main():
    ...
    has_overrides = any([
        args.loss, args.dropout, args.lr, args.sampler,
        args.undersample, args.hidden, args.epochs,
        args.batch_size, args.scheduler, args.class_weights, args.seq_len
    ])
    if has_overrides:
    ...
    if args.stage == "train":
        ...
        run_stage("s07_eval.py", args.exp)
